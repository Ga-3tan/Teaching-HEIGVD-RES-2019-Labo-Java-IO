...
public class Application implements IApplication {
  ...
  void storeQuote(Quote quote, String filename) throws IOException {
    ...
    StringBuilder tagsDirectory = new StringBuilder();
    for (String x : tagsString) {
      tagsDirectory.append("/").append(x);
    }
    File file = new File(WORKSPACE_DIRECTORY + tagsDirectory.toString()+ "/" + filename + ".utf8");
    if (!file.exists()) {
      file.getParentFile().mkdirs();
    }
    PrintWriter writer = new PrintWriter(file, "UTF-8");
    writer.println(quote.getQuote());
    writer.close();
  }
